...
import dev.abstr3act.addon.utils.compassion.impl.EntityUtil;
import dev.abstr3act.addon.utils.compassion.impl.PlaceData;
...
public class BlockUtil {
    ...
    public static boolean isSafeBlock(BlockPos pos) {
        return MeteorClient.mc.world.getBlockState(pos).isReplaceable()
            && MeteorClient.mc.world.getFluidState(pos).isEmpty()
            && !MeteorClient.mc.world.getBlockState(pos).isOf(Blocks.POWDER_SNOW);
    }

    public static boolean checkCanMove(
        Vec3d fromVec, Vec3d toVec, Map<BlockPos, BlockState> extraPosMap, dev.abstr3act.addon.utils.compassion.impl.PlayerUtil.PlayerState state
    ) {
    ...
    public static boolean collidesWithAnyBlock(
        Box bbox, Map<BlockPos, BlockState> extraPosMap, dev.abstr3act.addon.utils.compassion.impl.PlayerUtil.PlayerState condition
    ) {
    ...
    public static BlockPos findVclipHole(BlockPos vec, double vclip) {
        if (vclip > 0.0) {
            for (int i = (int) vclip + vec.getY(); i >= vec.getY(); i--) {
                BlockPos vec1 = new BlockPos(vec.getX(), i, vec.getZ());
                BlockPos vecUp = vec1.up(1);
                if (isSafeBlock(vec1) && isSafeBlock(vecUp)) {
                    return vec1;
                }
            }
        }
        ...
        if (vclip < 0.0) {
            for (int ix = (int) vclip + vec.getY(); ix <= vec.getY(); ix++) {
                BlockPos vec2 = new BlockPos(vec.getX(), ix, vec.getZ());
                BlockPos vecUp = vec2.up(1);
                if (isSafeBlock(vec2) && isSafeBlock(vecUp)) {
                    return vec2;
                }
            }
        }
    ...
    public static boolean checkCanMove(Vec3d fromVec, Vec3d toVec) {
        return isSafeBlock(BlockPos.ofFloored(fromVec)) && isSafeBlock(BlockPos.ofFloored(toVec));
    }

    public boolean checkNoPosCollie(Vec3d vec, Map<BlockPos, BlockState> extraPosMap, dev.abstr3act.addon.utils.compassion.impl.PlayerUtil.PlayerState state) {
        return collidesWithAnyBlock(state.getBBox(vec), extraPosMap, state);
    }

    public boolean checkNoPosCollie(
        double x, double y, double z, Map<BlockPos, BlockState> extraPosSet, dev.abstr3act.addon.utils.compassion.impl.PlayerUtil.PlayerState state
    ) {
